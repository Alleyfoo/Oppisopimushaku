...
WEBSHOP: dict[str, int] = {
    ...
    "11": 2,
    "13": 2,
    "14": 2,  # food & textile mfg
    "20": 2,
    "22": 2,
    "23": 2,
    "24": 2,  # chemicals, plastics, metals
    "25": 2,
    "26": 2,
    "27": 2,
    "28": 2,  # metal products, electronics, machinery
    "29": 2,
    "31": 2,
    "32": 2,  # vehicles, furniture, misc mfg
    "33": 1,  # machine repair — spare-parts shop
    "49": 1,
    "52": 1,  # logistics — customer portal / booking
}
...
PIM: dict[str, int] = {
    ...
    "25": 3,
    "26": 3,
    "27": 3,  # product manufacturers
    "20": 2,
    "22": 2,  # process manufacturers
}
...
DATA: dict[str, int] = {
    ...
    "28": 2,
    "25": 2,
    "26": 2,  # manufacturing — production analytics
...
def score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    has_web = (best_web.notna() | df["website"].notna()).astype(int)
...
def top_per_axis(df: pd.DataFrame, n: int = 10) -> pd.DataFrame:
    ...
    for label, col in axes.items():
        sub = df.sort_values(col, ascending=False).head(n).copy()
        sub["lead_type"] = label
    ...
    combined = combined.sort_values(
        ["lead_type", "axis_score"], ascending=[True, False]
    )
    return combined
